...
SAS <-  read.csv(here("Data","field","SAS nutrients vs BEUTI.csv")) 
```
...
# statistically compare BEUTI during experimental periods
beutiexpdf <- 
  beuti %>%
    select(date, X34N) %>%
    filter(date >= as.Date("2024-03-29") & date <= as.Date("2024-05-01")) %>%
      mutate(season = "Early Spring") %>%
  bind_rows(
    beuti %>%
      select(date,X34N) %>%
      filter(date >= as.Date("2024-05-13") & date <= as.Date("2024-06-15")) %>%
       mutate(season = "Late Spring")
  ) %>%
  na.omit()

t.test(X34N~season, data = beutiexpdf)

# test vs sas 
SAS %>%
  mutate(date = as.Date(Sampling_Date, "%m/%d/%Y")) %>%
  ggplot(aes(x = date, y = NO3)) +
    geom_smooth(aes(x = date, y = NO3),method = "loess", 
              color = "salmon4", size = 1) +
    geom_smooth(aes(x = date, y = BEUTI_34),method = "loess", 
              color = "blue", size = 1)+
    geom_point(aes(x = date, y = NO3), color = "salmon4", alpha = 0.2) +
    geom_point(aes(x = date, y = BEUTI_34), color = "blue", alpha = 0.2) +
    theme_bw()+
    theme(
      strip.background = element_rect(fill = "white"),
      text = element_text(size = 18),
...
```{r snail-subset}
# sub-sample for the general range of late spring
...
# one outlier, but otherwise largest snail is 0.0516

tegresposubdf <- tegrespodry %>%
  filter(dry_weight <= 0.0516)

# is it still normal after subsetting?
shapiro.test(tegresposubdf$Respiration)

#try with shell length 13-15mm
...
shell_lengths <- list(a = (tegrespodry_new %>% filter(experiment == "a"))$shell_length_mm, b = (tegulaSizes_new %>% filter(experiment == "b"))$shell_length_mm)
...
shell_lengths_sub <- list(a = (tegresposubdf %>% filter(experiment == "a"))$shell_length_mm, b = (tegresposubdf %>% filter(experiment == "b"))$shell_length_mm)
...
densityplot + subsetdensityplot + plot_layout(guides = "collect", axes = "collect")

#ggsave("Snail density plots.png", path = here("Output"), width = 8, height = 4)
```
