...
# predict using model
tegresposubdf <- tegresposubdf %>%
  mutate(predicted_res = predict(avg_res_model, newdata = tegresposubdf))

predictions <- predict(avg_res_model, newdata = tegresposubdf, interval = "confidence")
...
calc_new <- left_join(calc_new, tegulaSizes_new, by = c("experiment","ID"))
...
ggsave("effect sizes.png", path=here("Output"), width = 10, height = 8)
