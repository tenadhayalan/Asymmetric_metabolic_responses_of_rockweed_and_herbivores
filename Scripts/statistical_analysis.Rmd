---
title: "Asymmetric metabolic responses of rockweed and herbivorous snails reveal seasonal vulnerabilities to warming and acidification"
...
shellcalc <- read_csv(here("Data", "TA", "shell_NEC.csv")) # blank shell calcification
aprilpH <- read_csv(here("Data","pH_temp","ph_temp_early_spring.csv")) # early spring pH
aprilcarb <- read_csv(here("Data","pH_temp","carb_early_spring.csv")) # early spring background carbonate parameters
aprilTeg <- read_csv(here("Data","growth","snail_early_spring.csv")) # early spring snail sizes
maypH <- read_csv(here("Data","pH_temp","ph_temp_late_spring.csv")) # late spring pH
maycarb <- read_csv(here("Data", "pH_temp", "carb_late_spring.csv")) # may background co2 parameters
mayTeg <- read_csv(here("Data","growth","snail_late_spring.csv")) # late spring snail sizes
Sample.Info <- read_csv(here("Data","PR","Metadata_combined.csv")) # respo metadata both months
respo_dry_clean <- read.csv(here("Data", "PR","Physiological_rates.csv")) # respo both months
...
ggsave("Fig 1C.png", path=here("Output"))
```
...
ggsave("Fig 1B.png", path = here("Output"))

# what were temperatures two weeks before collection like?

# early spring = 15.05, sd = 1.01
temp %>%
  filter(date >= as.Date("2024-03-15") & date <= as.Date("2024-03-29")) %>%
           summarize(avg_temp = mean(sea_water_temperature, na.rm = TRUE), 
                     temp_sd = sd(sea_water_temperature, na.rm = TRUE))

# late spring = 15.91, sd = 0.51
temp %>%
  filter(date >= as.Date("2024-04-30") & date <= as.Date("2024-05-13")) %>%
           summarize(avg_temp = mean(sea_water_temperature, na.rm = TRUE),
                     temp_sd = sd(sea_water_temperature, na.rm = TRUE))

tempdf <- temp %>%
  filter(date >= as.Date("2024-03-15") & date <= as.Date("2024-03-29")) %>%
  mutate(season = "Early Spring") %>%
  bind_rows(
    temp %>%
      filter(date >= as.Date("2024-04-30") & date <= as.Date("2024-05-13")) %>%
  mutate(season = "Late Spring")
  ) %>%
  na.omit()

# statistically compare the temperature the week before collection for both experiments

t.test(sea_water_temperature~season, data = tempdf)
```
...
ggsave("Fig 1A.png", path=here("Output"))

# statistically compare BEUTI before early and late spring
beutidf <- 
  beuti %>%
    select(date, X34N) %>%
    filter(date >= as.Date("2024-03-15") & date <= as.Date("2024-03-29")) %>%
...
t.test(X34N~season, data = beutidf)
...
ggsave("Fig 1F.png", path=here("Output"))
```
...
ggsave("Fig 1D.png", path=here("Output"))
```
...
ggsave("Fig S1.png", path=here("Output"), width = 12, height = 6) # save to output folder
...
tankphgraph + tanktempgraph

ggsave("Fig S3.png", path=here("Output"), width = 12, height = 6)
```
...
### Set-up dataframes
```{r snail-respo-setup, echo=FALSE}
# merging snail respo data

## select only snail respiration rates
tegrespodry <- respo_dry_clean %>%
  filter(Species == "Tegula") %>% # filter out rockweeds
  mutate(ID = as.factor(ID))

## select only snail metadata
Teg.Sample.Info.New.pH <- Sample.Info.New %>%
  filter(Species == "Tegula") %>%
  select("ID", "block", "pH", "Date", "experiment")

## merge metadata and respiration rates
tegrespodry <- merge(tegrespodry, Teg.Sample.Info.New.pH, 
                     by = c("ID", "block", "experiment"), all.x = TRUE) %>%
              # making wet a number
               mutate(initial_wet_weight = as.numeric(initial_wet_weight)) %>%
              # making temp a factor
...
t.test(shell_length_mm~experiment, data = tegulaSizes)
t.test(final_dry_weight~experiment, data = tegulaSizes)
```

### Subsetting snail sizes
```{r snail-subset}
# sub-sample for the overlapping size range (shell length 13-15mm)
tegulaSizes_new <-bind_rows(
  tegulaSizes %>%
    filter(experiment == "Early spring") %>%
    mutate(experiment = "a"),
  tegulaSizes %>%
    filter(experiment == "Late spring") %>%
    mutate(experiment = "b")
  ) %>%
  mutate(ID = as.factor(snail_ID)) %>%
  select(-pH, -snail_ID)

tegrespodry_new <- left_join(tegrespodry, tegulaSizes_new, by = c("experiment","ID", "tank"))

# filter out shell lengths between 13-15 mm
tegresposubdf <- tegrespodry_new %>%
  filter(shell_length_mm >= 13 & shell_length_mm <= 15)

# how much did shell length densities overlap originally?
shell_lengths <- list(a = (tegrespodry_new %>% filter(experiment == "a"))$shell_length_mm, 
                      b = (tegulaSizes_new %>% filter(experiment == "b"))$shell_length_mm)

## density overlap graph
overlap(shell_lengths)
densityplot <- 
  final.plot(shell_lengths) +
  scale_fill_manual(values = c("#540D6E", "#0EAD69"),
                    labels = c("Early spring", "Late Spring"))+
  scale_color_manual(values = c("#540D6E", "#0EAD69"),
                     labels = c("Early spring", "Late Spring"))+
  theme(strip.text.x = element_blank())+
  annotate("text", x = 17.25, y = 0.52, label = "0.57")+
  labs(x = "Shell length (mm)",
       y = "Density",
       a = "Early spring",
       b = "Late spring")

# how much do shell length densities overlap in the 13-15 mm range?
shell_lengths_sub <- list(a = (tegresposubdf %>% filter(experiment == "a"))$shell_length_mm,
                          b = (tegresposubdf %>% filter(experiment == "b"))$shell_length_mm)

## density overlap graph
overlap(shell_lengths_sub)
subsetdensityplot <- 
  final.plot(shell_lengths_sub) +
  scale_fill_manual(values = c("#540D6E", "#0EAD69"),
                    labels = c("Early spring", "Late Spring"))+
  scale_color_manual(values = c("#540D6E", "#0EAD69"),
                     labels = c("Early spring", "Late Spring"))+
  annotate("text", x = 14.88, y = 1.17, label = "0.69")+
  theme(strip.text.x = element_blank())+
  labs(x = "Shell length (mm)",
       y = "Density",
       a = "Early spring",
...
initialsizeplot + initialshellplot + plot_layout(guides = "collect", axes = "collect_x") + densityplot + subsetdensityplot + plot_layout(guides = "collect", axes = "collect")

ggsave("Fig S2.png", path = here("Output"), width = 8, height = 8)
```

### Average models for each month
```{r teg-respo}
...
#### Average models for each month (subsetted in supplement)
```{r teg-respo-supp}

# calculate averages per treatment
tegrespo_avg_sub <- tegresposubdf %>%
  group_by(pH, temp_treatment, experiment) %>%  # group by temp and pH
  summarize(
    avg_res = mean(Respiration, na.rm = TRUE)  # calculate mean respo rate
  )
...
# early spring avg model
avg_res_model_april_sub <- lm(avg_res ~ pH * temp_treatment, data = tegrespo_avg_sub %>% 
                            filter(experiment == "a"))
anova(avg_res_model_april_sub) # nothing sig


# late spring avg model
avg_res_model_may_sub <- lm(avg_res ~ pH * temp_treatment, data = tegrespo_avg_sub %>% 
                          filter(experiment == "b"))
anova(avg_res_model_may_sub) # temp sig
```
...
tegrespo_avg <- tegrespo_avg %>%
  ungroup() %>%
  # make temp a number
  mutate(temp_num = as.numeric(as.character(temp_treatment))) %>% 
  # scale the values to calculated standardized effects
  mutate(pH_scale = as.numeric(scale(pH)), 
         temp_scale = as.numeric(scale(temp_num)))

# april model using scaled data
...
calc_new <- calc %>%
  ...
  mutate(ID = as.factor(ID)) %>%
  # join with metadata
  left_join(Teg.Sample.Info.New.pH %>% 
            dplyr::select(ID,pH,experiment)) %>%
  # make temp and exp a factor
  mutate(temp_treatment = as.factor(temp_treatment)) %>%
  mutate(experiment = as.factor(experiment))

calc_new <- calc_new %>%
  filter(!deltaTA > mean(umol.cm2.hr, na.rm = TRUE) + 3 * sd(umol.cm2.hr, na.rm = TRUE)) 
```

### Average models on full data (main)
```{r snail-calc}
# calculate averages 
calc_avg <- calc_new %>%
  group_by(pH, temp_treatment, experiment) %>%  # group by temp and pH
...
### Average models on subsetted data (supplement)
```{r snail-calc-sub}
calc_sub <- left_join(calc_new, tegulaSizes_new, by = c("experiment","ID"))
calc_sub <- calc_sub %>%
  filter(shell_length_mm >= 13 & shell_length_mm <= 15)

# calculate averages 
calc_avg_sub <- calc_sub %>%
  group_by(pH, temp_treatment, experiment) %>%  # group by temp and pH
  summarize(
    avg_calc = mean(umol.cm2.hr, na.rm = TRUE)  # calculate mean calc rate
  )
...
# early spring avg model
avg_calc_model_april <- lm(avg_calc ~ pH * temp_treatment, data = calc_avg_sub %>% filter(experiment == "a"))
anova(avg_calc_model_april) # nothing sig
 
# late spring avg model
avg_calc_model_may <- lm(avg_calc ~ pH * temp_treatment, data = calc_avg_sub %>% filter(experiment == "b"))
anova(avg_calc_model_may) # pH and temp sig
...
# April - run  model on the scaled data for april
...
std_coefs<-as_tibble(effectsize(tegmodel_cal_april)) %>% # get the effect sizes for early spring
  mutate(month = "Early spring") %>% # add the month
  left_join(tidy(tegmodel_cal_april) %>%
              dplyr::select(Parameter = term, p.value)) %>% # bring in the p-values from the summary of the model
  bind_rows(as_tibble(effectsize(tegmodel_cal_may)) %>% # bind it to the same for late spring
              mutate(month = "Late spring") %>%
...
std_coefs %>%
  ...
  geom_vline(xintercept = 0, lty = 3)+
  geom_point(size = 3, 
             position = position_dodge(width = 0.2))+ # offset the points
  geom_errorbarh(aes(xmin = CI_low, xmax = CI_high), 
                 height = 0.1, position = position_dodge(width = 0.2))+
  labs(x = "Standardized Effect Size \n Calcification", 
       y = "",
...
### Shell calcification (supplemental figure)
```{r}
shellcalc %>%
  filter(is.na(sample_type)) %>%
  distinct() %>%
  ggplot(aes(x=pH_treatment, y=umol.hr, color = as.factor(temp_treatment),fill=as.factor(temp_treatment)))+
  geom_point(alpha = 1)+
  theme_bw() +
  theme(strip.background = element_rect(fill = "white"),
        text = element_text(size = 16)) +
...
anova(rwmodelGP_Pa) # pH sig

# late spring
rwmodelGP_Pb <- lm(avg_gp ~ pH * temp_treatment * predator, data=rwrespo_avg_p %>% filter(experiment == "b"))
anova(rwmodelGP_Pb) # pH sig, temp:predator sig

# how much greater was GP at 20?
rwrespo_avg_p %>%
  filter(experiment == "b") %>%
...
ggsave("Fig. 5.png", path = here("Output"), width = 8, height = 7)
```
...
# April - run model on the scaled data for early spring
...
std_coefs<-as_tibble(effectsize(rwmodel_gp_april)) %>% # get the effect sizes for early spring
  mutate(month = "Early spring") %>% # add the month
  left_join(tidy(rwmodel_gp_april) %>%
              dplyr::select(Parameter = term, p.value)) %>% # bring in the p-values from the summary of the model 
  bind_rows(as_tibble(effectsize(rwmodel_gp_may)) %>% # bind it to the same for late spring
              mutate(month = "Late spring") %>%
  ...
  mutate(Parameter = case_when(Parameter == "temp_scale" ~"Temperature",
                               ...
                               Parameter == "pH_scale:temp_scale"~"pH:Temperature")) %>% # make names prettier
  mutate(Parameter = factor(Parameter, levels = c("pH:Temperature","Temperature","pH"))) %>% # put it in the order I want
  mutate(significant = ifelse(p.value < 0.055, 1, 0.5)) # use this to change the alpha
...
# avg model for plot
avg_gp_model <- lm(avg_gp ~ pH * temp_treatment * experiment, data = rwrespo_avg_p)

# predict using model
rwrespo_clean_plot <- rwrespo_clean %>%
  mutate(predicted_gp = predict(avg_gp_model, newdata = rwrespo_clean))

predictions <- predict(avg_gp_model, newdata = rwrespo_clean_plot, interval = "confidence")
...
std_coefs<-as_tibble(effectsize(rwmodel_gp_april)) %>% # get the effect sizes for april
  ...
  left_join(tidy(rwmodel_gp_april) %>%
              dplyr::select(Parameter = term, p.value)) %>% # bring in the p-values from the summary of the model 
  bind_rows(as_tibble(effectsize(rwmodel_gp_may)) %>% 
              mutate(month = "Late spring") %>%
              left_join(tidy(rwmodel_gp_may) %>%
                          dplyr::select(Parameter = term, p.value))) %>%
  filter(Parameter != "(Intercept)") %>% # remove the intercepts for the STD Coef plot
  ...
  mutate(significant = ifelse(p.value < 0.055, 1, 0.5)) # use this to change the alpha of points
...
### Respiration without consumer for plot
```{r rw-respo-phtemp}
# avg model for plot
rwrespo_avg <- rwrespo_clean %>%
  group_by(pH, temp_treatment, experiment) %>%  # group by temp and pH
...
(rwrespoplot + theme(plot.margin = unit(c(0,30,0,0), "pt"))) + rwgpplot + 
  (tegrespoplot + theme(plot.margin = unit(c(0,30,0,0), "pt"))) + tegcalcplot + 
  plot_layout(guides = "collect", axes = "collect_y")

ggsave("Fig. 4.png", path=here("Output"), width = 10, height = 8)
...
avgrwrespoplot + avgrwgpplot + avgtegrespoplot + avgcalcplot + plot_layout(guides = "collect", axes = "collect_x")

ggsave("Fig. 3.png", path=here("Output"), width = 10, height = 8)

rwrespopredplot+ rwgppredplot + plot_layout(guides="collect", axes = "collect_y")

ggsave("Fig. S5.png", path=here("Output"), width = 10, height = 4)
